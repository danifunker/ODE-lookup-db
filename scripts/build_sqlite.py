...
def main() -> int:
    ...
    # read_winworld_disc_images() prefers the committed JSONL and falls back
    # to walking the NAS archives dir. Either source is enough.
    from ode_lookup_db.db import WINWORLD_DISC_IMAGES_JSONL                 # noqa: E402
    disc_images = None
    if ww is not None:
        if WINWORLD_DISC_IMAGES_JSONL.exists() or WINWORLD_ARCHIVES_DIR.is_dir():
            disc_images = read_winworld_disc_images()
        else:
            logging.info("no disc_images.jsonl and no archives dir; "
                         "disc_image table will be empty")
